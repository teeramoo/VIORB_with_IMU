...
//    cv::Mat GrabImageMonoVIwithCamPose(const cv::Mat &im, const std::vector<IMUData> &vimu, const double &timestamp, const cv::Mat &BotCamPose);
...
public:
	...
	void SetSaveFolder(string savefoldername);
	void SaveAndExit();
...
public:
    ...
    void SetForcedInitFrame(const int &forceFrame);
    int forcedInitFrame;
    void SetForceKFMatrix(cv::Mat &forcedKFmatrix);
    cv::Mat mForcedKF;
    cv::Mat mForcedFrame;
    void SetForceFrameMat(cv::Mat &ForceFrameMat);
    bool isSecondSLAM;
    void SetSecondSLAM(const bool SecondSLAM);
    bool GetSecondSLAM();
    cv::Mat mCamBotToTop;
    void SetCamBotToTop(cv::Mat &_mCamBotToTop);
    bool TrackFromFirstSLAM();
...
protected:
  ...
  //  void Track(const cv::Mat &BotCamPose);
