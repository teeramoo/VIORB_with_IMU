...
bool LocalMapping::TryInitVIO(void)
{
    ...
    {
    	cout << "Keyframes: " << mpMap->KeyFramesInMap() << flush;
    	cout << endl;
    	cout << "mnLocalWindowSize: " << mnLocalWindowSize << flush;
    	cout << endl;
        return false;
    }
    ...
    if(!fopened)
    {
        ...
        cout << "ConfigParam::getTmpFilePath() : " << ConfigParam::getTmpFilePath() << endl;
   ...
   // cout<<"w:"<<endl<<w<<endl;
    ...
    if(mbFirstTry)
    {
        cout << "Im in mbFirstTry " << endl;
        mbFirstTry = false;
        mnStartTime = mpCurrentKeyFrame->mTimeStamp;
    }
    cout << "mpCurrentKeyFrame->mTimeStamp : " << setprecision(16) << mpCurrentKeyFrame->mTimeStamp << endl;
    cout << "mnStartTime : "<< setprecision(16) << mnStartTime << endl;
    cout << "mpCurrentKeyFrame->mTimeStamp - mnStartTime : "<< setprecision(16) << mpCurrentKeyFrame->mTimeStamp - mnStartTime << endl;
   /*
    if(mpCurrentKeyFrame->mTimeStamp - mnStartTime >= 100000.0)
    {
        bVIOInited = true;
    }
*/
    if(mpCurrentKeyFrame->mTimeStamp - mnStartTime >= 0.1)
    {
        bVIOInited = true;
    }

    cout << " time different is : " << mpCurrentKeyFrame->mTimeStamp - mnStartTime << endl;
...
void LocalMapping::Run()
{
        ...
        if(CheckNewKeyFrames())
        {
            ...
            if(!CheckNewKeyFrames() && !stopRequested())
            {
                ...
                if(mpMap->KeyFramesInMap()>2)
                {
                    if(!GetVINSInited())
                    {
                        cout << "*******************************************************Pure Bundle adjustment**************************************************************************************" << endl ;
                    ...
                    {
                        cout << "--------------------------------------------------Bundle adjustment with NavState----------------------------------------------------------------------------------" << endl ;
    ...
    cout << "navStateCounter is : " << navStateCounter << endl;
    SetFinish();
}
