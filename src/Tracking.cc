...
bool Tracking::TrackFromFirstSLAM()
    {
        // Create new ORB tracker without orientation checking (feature orientation estimates
...
//        ORBmatcher matcher(0.9,false);
    ORBmatcher matcher(0.9,false);
        ...
        cout << "mLastFrame.mTcw before update " << endl << mLastFrame.mTcw << endl;
         UpdateLastFrame();

    cout << "mLastFrame.mTcw after update " << endl << mLastFrame.mTcw << endl;


    // Get initial pose of this frame based on GPS+IMU or IMU
...
//           cv::Mat mCurrentTopCamToWorld = mCurrentFrame.mForcedFrame * mCamBotToTop;
 ...
 //           mCurrentFrame.SetPose(Tcl * mLastFrame.mTcw);
        ...
        else
        {
                cv::Mat Tcl = mCurrentFrame.mTcw * mLastFrame.mTcw.inv();
                mCurrentFrame.SetPose(Tcl * mLastFrame.mTcw);
                cout << "Checking at F2 E :************************((((((((((((((((((****************************************************" << endl;
                sleep(3);
        }

        // Initialize 2D->3D correspondences for the current frame

        fill(mCurrentFrame.mvpMapPoints.begin(),mCurrentFrame.mvpMapPoints.end(),static_cast<MapPoint*>(NULL));

        // Project points associated with map points from previous frame into current frame and find nearest correspondences

        int th=15;
        int nmatches = matcher.SearchByProjection(mCurrentFrame,mLastFrame,th,true);
        cout << "SBP 1 yields " << nmatches << " points" << endl;

        // If few matches, use a wider window search

        if(nmatches<100)
        {
            fill(mCurrentFrame.mvpMapPoints.begin(),mCurrentFrame.mvpMapPoints.end(),static_cast<MapPoint*>(NULL));
            nmatches = matcher.SearchByProjection(mCurrentFrame,mLastFrame,2*th,true);
            cout << "SBP 2 yields " << nmatches << " points" << endl;
        }

        cout << "Total_1 number of nmatches is : " << nmatches << endl;
...
#if 1
    cout << "Pose before optimization " << mCurrentFrame.mTcw << endl;
#endif
//    Optimizer::PoseOptimization(&mCurrentFrame, 4.0);
//    Optimizer::PoseOptimization(&mCurrentFrame);
#if 1
    cout << "Pose after optimization " << mCurrentFrame.mTcw << endl;
#endif

    // Discard outliers
    int nmatchesMap = 0;
    cout << "Total number of mCurrentFrame is : " << mCurrentFrame.N << endl;
    ...
    cout << "Total_2 number of nmatchesMap is : " << nmatchesMap << endl;
    if(!isSecondSLAM)
...
void Tracking::SetCamBotToTop(cv::Mat &_mCamBotToTop)
{
    mCamBotToTop = _mCamBotToTop ;
}

bool Tracking::GetSecondSLAM()
    {
        return isSecondSLAM;
    }

void Tracking::SetSecondSLAM(const bool SecondSLAM)
{
    isSecondSLAM = SecondSLAM;
}

void Tracking::SetForceFrameMat(cv::Mat &_mForcedFrame)
    {
        mForcedFrame = _mForcedFrame.clone();
      /*
        if(_mForcedFrame.empty())
             mForcedFrame = _mForcedFrame.clone();
        else
        {
            mForcedFrame = mCamBotToTop.clone() * _mForcedFrame.clone();
        }
      */
...
void Tracking::SetForceKFMatrix(cv::Mat &_mForcedKF)
{
    mForcedKF = _mForcedKF.clone();
}

void Tracking::SetForcedInitFrame(const int &forceFrame)
{
    forcedInitFrame = forceFrame;
}

bool Tracking::CheckInitialization()
{
    return isInitialize;
}
...
bool Tracking::TrackLocalMapWithIMU(bool bMapUpdated)
{
    ...
    cout << "Matched after IMU updated : " << mnMatchesInliers << endl;
...
bool Tracking::TrackWithIMU(bool bMapUpdated)
{
    ...
    cout << "total nmatches is : " << nmatches << endl;
    if(nmatches<20)
        return false;
    ...
    cout << "Matches in TrackWithIMU() >> " << nmatchesMap << endl;
...
IMUPreintegrator Tracking::GetIMUPreIntSinceLastKF(Frame* pCurF, KeyFrame* pLastKF, const std::vector<IMUData>& vIMUSInceLastKF)
{
    ...
    for(size_t i=0; i<vIMUSInceLastKF.size(); i++)
    {
        ...
        if(dt <= 0)
        {
            cerr<<std::fixed<<std::setprecision(6)<<"dt = "<<dt<<", this vs next time: "<<imu._t<<" vs "<<nextt<<endl;
            std::cerr.unsetf ( std::ios::showbase );                // deactivate showbase
        }
    }
    ...
    cv::Mat Tracking::GrabImageMonoVI(const cv::Mat &im, const std::vector<IMUData> &vimu, const double &timestamp)
{
    ...
    if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET)
    {
        ...
        mCurrentFrame.SetFrameNo(iFrameNo);
        if(isSecondSLAM) //if(!mForcedFrame.empty() && isSecondSLAM)
            mCurrentFrame.SetForcedFrame(mForcedFrame);
    }
    ...
    return mCurrentFrame.mTcw.clone(); // Return world to camera transformation matrix
}
    ...
    mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0)
{
    ...
    //fSettings["OutputFolder"] >> _sSaveFolder;
...
void Tracking::SetFrameNumber(int fn)
{
	this->iFrameNo = fn;
}


void Tracking::SetSaveFolder(string savefoldername)
{
	this->_sSaveFolder = savefoldername;
}
...
//		sleep(10);
                ...
                {mbRelocBiasPrepare = false;
                    ...
                    if(mbRelocBiasPrepare)
                    {
                        ...
                        cout << "bOK at 10E: " << bOK << endl;
                    }
                    else
                    {
                        bOK = TrackWithIMU(bMapUpdated);
                        cout << "bOK at 9E: " << bOK << endl;
                        if(!bOK)
                            bOK = TrackReferenceKeyFrame();
                        cout << "bOK at 8E: " << bOK << endl;
...
#endif
                {
                    ...
                    if(isSecondSLAM)
                    {
                        bOK = TrackFromFirstSLAM();
                        cout<< "bOK at FirstSLAM: " << bOK << endl;
                    }
                    else if(mVelocity.empty() || mCurrentFrame.mnId<mnLastRelocFrameId+2)
                    {
                        bOK = TrackReferenceKeyFrame();
                        cout << "bOK at 7E: " << bOK << endl;
                    }
                    else
                    {
                        bOK = TrackWithMotionModel();
                        cout << "bOK at 6E: " << bOK << endl;
                        if(!bOK)
                        {
                            bOK = TrackReferenceKeyFrame();
                        	cout << "bOK at 5E: " << bOK << endl;
                        }
                    }
                }
            }
            ...
            {
            	// SKIP THIS TO CONTROL SEG FAULT. NO TIME TO CHECK FULL SEG FAULT CAUSE CURRENTLY

                bOK = Relocalization();
                if(bOK) cout<<"Relocalized. id: "<<mCurrentFrame.mnId<<endl;
                cout << "bOK at 4E: " << bOK << endl;
                //bOK = false;
            }
        }
        else
        ...
        if(!mbOnlyTracking)
        {
            if(bOK)
            {
                ...
                bOK = TrackLocalMap();
                cout << "bOK at 3E: " << bOK << endl;
#else
                if(!mpLocalMapper->GetVINSInited())
                {
                    bOK = TrackLocalMap();
                    cout << "bOK at 2E: " << bOK << endl;
                }
                else
                {/*
                    if(mbRelocBiasPrepare)
                    {
                        // 20 Frames after reloc, track with only vision
                        bOK = TrackLocalMap();
                        cout << "bOK at 1E: " << bOK << endl;
                    }
                    else
                    {
                        bOK = TrackLocalMapWithIMU(bMapUpdated);
                        cout << "bOK at E: " << bOK << endl;
                    }
                    */
                }
#endif
        ...
        {
            mState=LOST;
            cout << "State lost here: add Frames to re-compute IMU bias after reloc" << endl;
            mpSystem->Reset();
        ...
        if(bOK)
        {
            ...
            if(!mLastFrame.mTcw.empty())
            {
                ...
                if(isSecondSLAM)
                    cout << "mVelocity is : " << endl << mVelocity << endl;
            }
            else
                mVelocity = cv::Mat();
            ...
            if(NeedNewKeyFrame() || mbCreateNewKFAfterReloc)
                CreateNewKeyFrame();
            //cout << "**************** KEYFRAME : " << mpMap->KeyFramesInMap() << endl;
            if(vFrameID.size() < mpMap->KeyFramesInMap())
            {
            	if(vFrameID.size() == 0)
            	{

            		size_t itempsize = mpMap->KeyFramesInMap() - vFrameID.size();
		        	for(size_t j = 0; j < itempsize; j++)
		        	{
		        		vFrameID.push_back(this->iFrameNo);
		        	}
		        }
		        else
		        {
            ...
            else if(vFrameID.size() > mpMap->KeyFramesInMap())
            {
            	vFrameID.pop_back();
            }
            /*
            ...
            for(size_t i=0; i<vpKFs.size(); i++)
        	{
            	...
            	cout << "Keyframe: " << i << " Frame: " << vFrameID[i] << endl;
            	cout << Tcw << endl;
        	}
        	*/
            // Clear flag
        ...
        {
            cout << "Previous Frame is : " << endl << mLastFrame.mTcw << endl;
            cout << "Current Frame is : " << endl << mCurrentFrame.mTcw << endl;
...
void Tracking::MonocularInitialization()
{
    ...
    if(iFrameNo == forcedInitFrame-1  && isSecondSLAM)
    {
        bForceInit = true;
        cout << "bForceInit is set as True." <<endl;
    }
        ...
        if (!mpInitializer) {
            ...
            // Set Reference Frame
            if (mCurrentFrame.mvKeys.size() > 100  || bForceInit) {
    ...
    {
        // Try to initialize
        if((int)mCurrentFrame.mvKeys.size()<=100 && !bForceInit) // if((int)mCurrentFrame.mvKeys.size()<=100)
        {
            delete mpInitializer;
        ...
        //Added by Teera, edited from Dr. Matt's code
            //Calculate IMU distance
        /*
        if(isSecondSLAM && !mInitialFrame.mForcedFrame.empty() && !mCurrentFrame.mForcedFrame.empty())
        {
            cv::Mat mTic = mInitialFrame.mForcedFrame * mCurrentFrame.mForcedFrame.inv();
            float distIMU = cv::norm(mTic.rowRange(0,3).col(3));
            cout << "distIMU between previous frame and current frame is : " << distIMU << endl;
        }
*/
        ...
        if(nmatches<100 && !bForceInit)
        {
            delete mpInitializer;
            mpInitializer = static_cast<Initializer*>(NULL);
            return;
        }
        cout << "Initialize with nmatches: " << nmatches << endl;
...
//        if(mpInitializer->Initialize(mCurrentFrame, mvIniMatches, Rcw, tcw, mvIniP3D, vbTriangulated))
        if(mpInitializer->Initialize(mCurrentFrame, mvIniMatches, Rcw, tcw, mvIniP3D, vbTriangulated, bForceInit))
        {
            ...
            // Set Frame Poses
                ...
                Rcw.copyTo(Tcw.rowRange(0,3).colRange(0,3)); // Rcw and Tcw are obtained from H or F reconstruction
                ...
                {
                    cout << "Create map without mForcedFrame" << endl;
...
bool Tracking::TrackLocalMap()
{
    ...
    if(isSecondSLAM)
        Optimizer::PoseOptimization(&mCurrentFrame, 4.0);// cout << "No optimization please" << endl; //Optimizer::PoseOptimization(&mCurrentFrame, 4.0);
    else
        Optimizer::PoseOptimization(&mCurrentFrame);
    ...
    // More restrictive if there was a relocalization recently
    if(mCurrentFrame.mnId<mnLastRelocFrameId+mMaxFrames && mnMatchesInliers<50 && !isSecondSLAM)
    {
    	cout << "Fail by mnMatchesInliers: " << mnMatchesInliers << endl;
        return false;
	}

    if(mnMatchesInliers<30 && !isSecondSLAM)
    {
    	cout << "Fail due to inlier: " << mnMatchesInliers << endl;
        return false;
	}
    else
    {
        cout << "Total number of inliers is : " << mnMatchesInliers << endl;
        return true;
    }
...
vector<KeyFrame*> Tracking::GetAllKeyFrames()
{
    const vector<KeyFrame*> vpKFs = mpMap->GetAllKeyFrames();
    return vpKFs;
}

vector<int> Tracking::GetFrameID()
{
    return vFrameID;
}
...
void Tracking::SaveKeyFrameInfo()
{
	stringstream ss;
	ss << _sSaveFolder << "KeyFrameInfo_" << _iFileIndex << ".txt";
	string sFileName = ss.str();
	cout << endl << "Saving KeyFrame Info to " << sFileName << " ..." << endl;

    if(mpMap)
    {

		int iKFsize = mpMap->KeyFramesInMap();
		if(iKFsize > 0 && vFrameID.size() != NULL)
		{			
			const vector<KeyFrame*> vpKFs = mpMap->GetAllKeyFrames();
            ofstream f;
			...
			{
				KeyFrame* pKF = vpKFs[i];
				cv::Mat Tcw = pKF->GetPoseInverse();
				cv::Mat Twc = pKF->GetPose();
				cv::Mat Ow = pKF->GetCameraCenter();

                f << "Keyframe: " << i << " Frame: " << pKF->iFrameNumber << " Frame from vFrameID : " << vFrameID[i] << flush;
                f << endl;
				f << "Tcw:" << flush;
				f << endl;
				f << Tcw << flush;
    ...
    cout << endl << "KeyFrame info saved!" << endl;
}

void Tracking::SaveMapPoints()
{
	stringstream ss;
	ss << _sSaveFolder << "MapPoints_" << _iFileIndex << ".pcd";
	string sFileName = ss.str();
	cout << endl << "Saving Map Points to " << sFileName << " ..." << endl;
	_iFileIndex++;
    ...
    for(size_t i=0; i<vpMPs.size(); i++)
    {
        ...
        f << setprecision(7) << pos.at<float>(0) << setprecision(7) << " " << pos.at<float>(1) << " " << pos.at<float>(2) << " 0"<< flush;
        f << endl;
    }

    f.close();
    cout << endl << "Map Points saved with " << iWCount << " points." << endl; 
}
...
void Tracking::Reset()
{
    ...
    SaveKeyFrameInfo();
    cout << "Printing Save Keyframe Info.............." << endl;
	SaveMapPoints();
